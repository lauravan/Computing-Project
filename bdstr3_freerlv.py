...
#form factors are given by data sheet
# V3 = 0
# V8 = 0
# V11 = 0
...
#utilise a matrix formulation to find the energies of the bands
    ...
    #indexes the coordinates within the matrix
            ...
            if deltag.dot(deltag) == 3:
                energy += (V3 * strucfact(deltag))           
            
            if deltag.dot(deltag) == 8:
                energy += (V8 * strucfact(deltag))     

            if deltag.dot(deltag) == 11:
                energy += (V11 * strucfact(deltag))            
...
# print(matrix(2,'fcc',numpy.array([1,0,0])))
############################################################
...
#test
print(eigenvalues(2,'fcc',numpy.array([2,0,0])))
...
for i in range(len(alpha)):
#loop that generates k vectors in the lambda direction
...
for i in range(len(alpha)):
#loop that generates k vectors in the k direction
...
for rlv in range(len(RLVS(N, 'fcc'))):
    ...
    for x in range(len(kxs)):
        ...
        xenergy = eigenvalues(N, 'fcc', kxs[x])
    ...
    for l in range(len(kls)):
        ...
        lenergy = eigenvalues(N, 'fcc', kls[l])
